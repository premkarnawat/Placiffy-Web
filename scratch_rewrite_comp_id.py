  ...
  const handleMessageCompany = async () => {
    ...
    try {
      ...
      if (adminConvs && adminConvs.length > 0) {
        const convIds = adminConvs.map((c: any) => c.conversation_id);
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {jobs.map((j: any) => (
                      <tr key={j.job_id} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-4">
                           <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${j.status === 'open' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{j.status || 'ACTIVE'}</span>
                        </td>
          ...
          <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-700">{company.created_at ? company.created_at.split('T')[0] : 'Unknown'}</p>
              </div>
            </div>
          </div>
        </div>
