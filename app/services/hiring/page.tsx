...
import { Briefcase, ArrowLeft, ChevronRight, Check, Sparkles } from 'lucide-react';

export default function HiringServicePage() {
  const steps = [
    { title: 'Job Creation', desc: 'Define your role requirements, salary bounds, and skill metrics in our workspace.' },
    { title: 'AI Matching', desc: 'Our semantic database matches applicants and identifies top-tier profiles.' },
    { title: 'Shortlisting', desc: 'Placify parses and shortlists candidates with verified passports and scores.' },
    { title: 'Interview', desc: 'We coordinate scheduling, assessments, and technical evaluation interviews.' },
    { title: 'Hiring', desc: 'Onboard pre-vetted, high-trust candidates with guaranteed credentials.' }
  ];
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <main className="pt-32 pb-24 max-w-4xl mx-auto px-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 border border-blue-100 text-blue-700 text-xs font-bold uppercase rounded-full mb-6">
          <Briefcase size={14} />
          <span>Assisted Sourcing</span>
        </div>

        <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight mb-6">
          Placify Hiring Service
        </h1>
        <p className="text-lg text-slate-600 leading-relaxed mb-12">
          Save hundreds of hours by letting Placify recruit your engineering team. We assist in matching, verifying, interviewing, and finalizing candidates for your open roles.
        </p>

        {/* Workflow steps vertical list */}
        <h3 className="text-xl font-bold text-slate-950 mb-8 border-b border-slate-100 pb-3">Service Pipeline Workflow</h3>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h4 className="font-bold text-slate-900 text-lg mb-1">Ready to scale your team?</h4>
            <p className="text-slate-500 text-xs font-medium">Get in touch with us to set up your assisted search dashboard.</p>
          </div>
