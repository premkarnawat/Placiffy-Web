...
export default function PassportsAdmin() {
  ...
  const filtered = passports.filter(p => (p.candidates?.full_name || '').toLowerCase().includes(searchTerm.toLowerCase()) || (p.candidates?.users?.email || '').toLowerCase().includes(searchTerm.toLowerCase()) || p.id.includes(searchTerm));
