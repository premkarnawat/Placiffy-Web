...
export default function PassportLedger() {
  ...
  const fetchPassports = async () => {
    try {
      ...
      const { data, error } = await supabase.from('passports')
        .select('*, candidates(first_name, last_name, email, profile_photo_url)')
  ...
  const handleRegenerate = async (passportId: string, candId: string) => {
    try {
      ...
      // Simulate ATS logic regeneration
      const newTrustScore = Math.floor(Math.random() * 20) + 70; // 70-90 score
      ...
      toast("success", "Success", "Passport regenerated with updated ATS scores.");
      fetchPassports();
    } catch (e: any) {
      toast("error", "Error", e.message);
    } finally {
  ...
  const handleInvalidate = async (passportId: string) => {
    try {
      setActionLoading(passportId);
  ...
  const filtered = passports.filter(p => {
    ...
    const candEmail = (p.candidates?.email || '').toLowerCase();
    return p.id.toLowerCase().includes(term) || candName.includes(term) || candEmail.includes(term) || p.candidate_id.toLowerCase().includes(term);
  });
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-bold text-blue-600">{p.activity_score || 0}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-gray-900 font-medium">{new Date(p.created_at).toLocaleDateString()}</div>
                    <div className="text-[10px] text-gray-500 mt-0.5">Updated: {new Date(p.updated_at || p.created_at).toLocaleDateString()}</div>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    No passports found matching your search.
                  </td>
                </tr>
              )}
            </tbody>
