...
import { MessageSquare, Search, Loader2, Send, Paperclip, CheckCircle2, AlertCircle, Building2 } from 'lucide-react';
...
export default function MessagesAdmin() {
  ...
  const resolveEntity = async (userId: string) => {
    // Check candidate first
    const { data: cand } = await supabase.from('candidates').select('full_name, profile_photo_url, headline').eq('user_id', userId).maybeSingle();
    if (cand) return { name: cand.full_name, avatar: cand.profile_photo_url, subtitle: cand.headline, type: 'candidate' };
    
    // Check company next
    const { data: comp } = await supabase.from('companies').select('name, logo_url, industry').eq('user_id', userId).maybeSingle();
    if (comp) return { name: comp.name, avatar: comp.logo_url, subtitle: comp.industry, type: 'company' };
    
    // Check user table fallback
    const { data: usr } = await supabase.from('users').select('name, email, role').eq('id', userId).maybeSingle();
    return { name: usr?.name || usr?.email || 'Unknown User', avatar: null, subtitle: usr?.role || 'user', type: usr?.role || 'user' };
  };
    ...
    try {
      ...
      const enriched = await Promise.all((convs || []).map(async (c) => {
        const parts = await Promise.all((c.conversation_participants || []).map(async (p: any) => {
           const entity = await resolveEntity(p.user_id);
           return { ...p, entity };
        }));
  ...
  const fetchMessages = async (convId: string) => {
    try {
      ...
      const enrichedMsgs = await Promise.all((data || []).map(async (m: any) => {
         const entity = await resolveEntity(m.sender_id);
         return { ...m, entity };
      }));
  ...
  const filtered = conversations.filter(c => {
    const participantNames = c.conversation_participants?.map((p:any) => p.entity?.name).join(' ') || '';
        ...
        <div className="w-1/3 border-r border-gray-100 flex flex-col bg-slate-50/50">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              filtered.map(c => {
                const participants = c.conversation_participants?.filter((p:any) => p.entity?.type !== 'admin' && p.role !== 'admin') || [];
                const title = participants.map((p:any) => p.entity?.name).join(' & ') || 'Internal Thread';
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50 flex flex-col">
                {msgLoading ? (
                  ...
                  messages.map(m => {
                    const isAdmin = m.entity?.type === 'admin' || m.sender_id === user?.id;
                    const isCandidate = m.entity?.type === 'candidate';
                    const isCompany = m.entity?.type === 'company';
                    
                    return (
                      <div key={m.id} className={`flex flex-col max-w-[80%] ${isAdmin ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          {m.entity?.avatar ? (
                             <img src={m.entity.avatar} className="w-4 h-4 rounded-full object-cover" alt=""/>
                          ) : isCompany ? (
