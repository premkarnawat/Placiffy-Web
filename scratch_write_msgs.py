...
export default function MessagesAdmin() {
  ...
  const fetchConversations = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('conversations').select('*, conversation_participants(user_id, users(email, role))').order('updated_at', { ascending: false });
  ...
  const fetchMessages = async (convId: string) => {
    try {
      setMsgLoading(true);
      const { data, error } = await supabase.from('messages').select('*, users(email, role)').eq('conversation_id', convId).order('created_at', { ascending: true });
      if (error) throw error;
      setMessages(data || []);
    } catch (e) {
      console.error(e);
        ...
        <div className="w-1/3 border-r border-gray-100 flex flex-col bg-slate-50/50">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              filtered.map(c => {
                const participants = c.conversation_participants?.filter((p:any) => p.users?.role !== 'admin') || [];
                const title = participants.map((p:any) => p.users?.email?.split('@')[0]).join(' & ') || 'Empty Thread';
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 mt-1 truncate">Thread ID: {c.id.split('-')[0]}</p>
                  </button>
                )
              })
            )}
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
                {msgLoading ? (
                  ...
                  messages.map(m => {
                    const isAdmin = m.users?.role === 'admin';
