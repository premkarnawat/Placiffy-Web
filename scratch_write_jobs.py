  ...
  const fetchJobs = async () => {
    try {
      ...
      const jobsWithCounts = await Promise.all((data || []).map(async (j) => {
        const { data: apps } = await supabase.from('applications').select('status').eq('job_id', j.job_id);
        const appList = apps || [];
        return { 
          ...j, 
                  ...
                  <tr key={j.job_id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        {j.companies?.logo_url && <img src={j.companies.logo_url} alt="" className="w-6 h-6 rounded border border-gray-200" />}
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3 text-center">
                        ...
                        <div className="w-px h-6 bg-gray-200"></div>
                        <div>
                          <p className="text-sm font-black text-red-600 leading-none">{j.rejected}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mt-1">Rej</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 pr-6 text-right space-x-1">
                      <button className="text-slate-400 hover:text-blue-600 p-1.5 rounded-lg hover:bg-blue-50 transition-colors" title="View"><Eye size={16}/></button>
                      <button className="text-slate-400 hover:text-amber-600 p-1.5 rounded-lg hover:bg-amber-50 transition-colors" title="Archive"><Archive size={16}/></button>
                      <button className="text-slate-400 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 transition-colors" title="Delete"><Trash2 size={16}/></button>
                    </td>
                  </tr>
                ))
              )}
