...
async function test() {
  const { data, error } = await supabase.from('candidates').select('id, user_id, headline, location, skills, experience_years, trust_score, is_verified, passports(id)').limit(10);
