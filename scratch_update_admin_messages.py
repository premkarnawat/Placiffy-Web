...
import { 
  ...
  Building2, User as UserIcon, Loader2, Download, CheckCircle2, Eye 
} from 'lucide-react';
  ...
  const fetchGlobalConversations = async () => {
    try {
      ...
      const [partsRes, msgsRes, candsRes, compsRes] = await Promise.all([
        ...
        supabase.from('companies').select('user_id, name, logo_url')
      ]);
      ...
      const enriched = convData.map(conv => {
        ...
        return {
          ...
          company: comp ? { name: comp.name, logo: comp.logo_url } : null,
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Monitor global communications and securely intervene.</p>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(conv => (
                <tr key={conv.id} className={`transition-colors ${conv.flagged ? 'bg-red-50/50' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="text-gray-900 line-clamp-2 max-w-sm">{conv.lastMessage || 'No messages yet (or only attachments)'}</div>
...
print("Admin Messaging List Updated")
