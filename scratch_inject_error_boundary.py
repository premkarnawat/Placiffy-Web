    ...
    if (this.state.hasError) {
      return (
        <div className="p-8 bg-red-50 text-red-900 border border-red-200 rounded-xl m-8">
          ...
          <pre className="whitespace-pre-wrap text-xs bg-red-100 p-4 rounded-lg">{this.state.error?.message}</pre>
          <pre className="whitespace-pre-wrap text-xs bg-red-100 p-4 rounded-lg mt-2">{this.state.error?.stack}</pre>
        </div>
      );
    }
