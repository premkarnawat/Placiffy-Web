...
export default function CreateJobWorkspace() {
  ...
  const [formData, setFormData] = useState({
    ...
    education_branch: '', education_min_percentage: 0, original_jd_url: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        salary_min: parseInt(formData.salary_min as any) || 0, salary_max: parseInt(formData.salary_max as any) || 0,
        salary_currency: formData.salary_currency, salary_negotiable: formData.salary_negotiable,
        experience_min: parseInt(formData.experience_min as any) || 0, experience_max: parseInt(formData.experience_max as any) || 0,
        ...
        education_branch: formData.education_branch, education_min_percentage: parseInt(formData.education_min_percentage as any) || 0,
        original_jd_url: formData.original_jd_url,
  ...
  const sections = [
    ...
    { id: 3, title: "Compensation", icon: DollarSign }, { id: 4, title: "Education", icon: GraduationCap },
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <label htmlFor="jd-upload" className="flex items-center gap-2 px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl text-sm font-bold cursor-pointer transition-colors border border-indigo-200">
                {extracting ? <Loader2 className="w-4 h-4 animate-spin"/> : <FileText className="w-4 h-4"/>} {extracting ? "Uploading & Analyzing..." : "Upload Original JD"}
              </label>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          {activeTab === 1 && (
            <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
              ...
              {formData.original_jd_url && (
                <div className="bg-green-50 text-green-700 p-4 rounded-xl flex items-center gap-2 text-sm font-bold border border-green-200">
                  <CheckCircle2 className="w-5 h-5"/> Original JD PDF Successfully Uploaded and Attached to Job Profile
                </div>
              )}
          ...
          {activeTab === 4 && (
             <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
                <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><GraduationCap className="text-purple-500"/> Education Constraints</h2>
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Minimum Education</label>
                    <select name="education_required" value={formData.education_required} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none"><option value="High School">High School</option><option value="Bachelor's Degree">Bachelor's Degree</option><option value="Master's Degree">Master's Degree</option></select>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Required Branch / Stream</label>
                    <input type="text" name="education_branch" value={formData.education_branch} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none" placeholder="e.g. Computer Science, Electronics..." />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Minimum Percentage (%)</label>
                    <input type="number" name="education_min_percentage" value={formData.education_min_percentage} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none" placeholder="e.g. 60" />
                  </div>
                </div>
             </motion.div>
          )}
          {activeTab === 5 && (
             <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
                <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Users className="text-rose-500"/> Hiring Constraints & Experience</h2>
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Notice Period Allowed</label>
                    <select name="notice_period_required" value={formData.notice_period_required} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none"><option value="Immediate">Immediate</option><option value="30 Days">30 Days</option><option value="90 Days">90 Days</option></select>
                  </div>
                </div>
             </motion.div>
          )}
          {activeTab === 6 && (
             <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
                <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><FileText className="text-slate-500"/> Full Description</h2>
                <textarea name="job_description" value={formData.job_description} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none h-48" placeholder="Enter the full Job Description for Candidates..." />
             </motion.div>
          )}
        </form>
      </div>
...
print("Deployed UI frontend tab separation and Original JD upload logic!")
